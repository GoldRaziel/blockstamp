...
export default function PortalPageIT() {
  ...
  // Se arriva ?session_id dallo Stripe handoff: conferma e pulisci la query
  useEffect(() => {
    ...
    return (
      <div className="relative text-sm">
        <button type="button" onClick={() => setOpen(v => !v)}
          ...
          <span>🌐 Lingua</span><span className="opacity-80">▼</span>
        </button>
        {open && (
  ...
  async function handleStamp() {
    ...
    } catch (e: any) { setError(e.message || "Errore imprevisto."); } finally { setBusy(false); }
  }
    ...
    <div className="beam beam-hero max-w-3xl mx-auto pt-6 pb-24">
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
        <div className="text-sky-100">
          <p className="text-sm leading-relaxed space-y-2 mb-1"><strong>ISTRUZIONI:</strong></p>
          <ul className="list-disc list-inside text-sky-100 space-y-1">
            <li>Nel file <strong>.zip</strong> creato in precedenza, inserisci un file di testo <strong>(.txt)</strong> con il <strong>codice SHA256</strong> generato in Home.</li>
            <li>Carica qui sotto il file <strong>.zip</strong> appena creato.</li>
            <li>Otterrai un file <strong>.ots</strong> come prova di registrazione.</li>
            <li>Salva il file <strong>.ots</strong> nella stessa cartella del tuo <strong>.zip</strong>.</li>
            <li>Salva in un altro file <strong>(.txt)</strong> il <strong>codice timbro</strong> generato qui sotto e mettilo nella cartella dello <strong>.zip</strong>.</li>
            <li>Tra <strong>48–72 ore</strong>, carica il <strong>.ots</strong> nella sezione <strong>VERIFY</strong> della Home.</li>
            <li>Riceverai il tuo <strong>numero di blocco</strong> sulla blockchain Bitcoin.</li>
            <li>Genera il tuo <strong>certificato di proprietà</strong> e salvalo in modo sicuro.</li>
          </ul>
          <p className="text-xs opacity-80 mt-2">Nota: in <em>VERIFY</em> carica il <strong>file .ots</strong>. Il <strong>codice timbro</strong> è un riferimento interno.</p>
        </div>

        <div className="flex items-start gap-3">
          <input ref={inputRef} id="file-it" type="file" accept=".zip" className="hidden"
                 onChange={(e) => setZipFile(e.target.files?.[0] || null)} />
          <div className="flex flex-col">
            <label htmlFor="file-it" className="px-4 py-2 rounded-md bg-amber-400 hover:bg-amber-300 text-black font-semibold cursor-pointer">
              SCEGLI FILE
            </label>
