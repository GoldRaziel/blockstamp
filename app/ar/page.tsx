...
export default function Page() {
  ...
  async function handleFile(f?: File | null) {
    ...
    try {
      ...
      console.error(e);
      setError(e?.message || "Errساعات durante il calcolo dell'impronta.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function startPayment() {
    try {
      ...
      if (!res.ok) throw new Error(json.error || "Errساعات pagamento");
      if (json.url) window.location.href = json.url;
    } catch (err: any) {
      console.error(err);
      setError(err?.message || "Errساعات durante il pagamento.");
    }
  }
  ...
  async function submitToServer() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || "Errساعات durante l'invio.");
      setServerHash(json.hash);
    } catch (e: any) {
      console.error(e);
      setError(e?.message || "Errساعات durante l'invio al server.");
    } finally {
      setBusy(false);
    }
  }
  ...
  Scegli file
    ...
    <p className="text-xs opacity-70 mt-2">
      ...
      <section id="procedura" className="space-y-5">
        <h2 className="text-3xl font-semibold">الطريقة</h2>
        <div className="grid md:grid-cols-3 gap-6">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
            <div className="text-sm opacity-70 mb-2">1 · حمّل ملفك</div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
            <div className="text-sm opacity-70 mb-2">3 · Prova e التحقق</div>
            <p className="text-sm opacity-90">
      ...
      <section id="guida" className="space-y-6">
        <h2 className="text-3xl font-semibold">الدليل: proteggi al meglio la tua idea</h2>
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            <h3 className="text-lg font-semibold mb-2">3 · Registrazione entro 72 ساعات</h3>
            <p className="text-sm opacity-90">
              Entro <b>72 ساعات</b> riceverai il codice di registrazione su blockchain Bitcoin che
